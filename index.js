...
class UtahScientificBps extends InstanceBase {
	...
	setupVariables() {
		...
		if (self.routerTablemap == undefined) self.routerTablemap = new Map()
	...
	doSC4init = function () {
		...
		if (self.src_idinfomap == undefined) self.src_idinfomap = new Map()

		if (self.dst_idinfomap == undefined) self.dst_idinfomap = new Map()

		if (self.src_nameidmap == undefined) self.src_nameidmap = new Map()

		if (self.dst_nameidmap == undefined) self.dst_nameidmap = new Map()

		if (self.router_statusmap == undefined) self.router_statusmap = new Map()
	...
	parseDevInfo(d) {
		...
		for (var j = 0; j < 10; j++) {
			if (d[pos] == 0) devName += ' '
			else devName = devName + String.fromCharCode(d[pos])
			pos++
		}
		pos += 2
	...
	parseLevelInfo(d) {
		...
		var pos = 1
		for (var j = 0; j < 10; j++) {
			if (d[pos] == 0) levelName += ' '
			else levelName = levelName + String.fromCharCode(d[pos])
			pos++
		}
		pos += 2
	...
	parseCommand_Unet(hdrcommand, dmsg) {
		...
		switch (hdrcommand) {
			...
			case 66: // RCP4_UNet_Matrix_Response_Chunk_End:
				console.log(
					self.label +
						'::router matrix status end, processed count: ' +
						self.statusCount +
						', map count: ' +
						self.router_statusmap.size,
				)
		...
		while (!done) {
			switch (self.datastate) {
				...
				case 1:
					{
						var dmsg = []
						if (hdrdatalen > 0) dmsg = dd.subarray(startpos, startpos + hdrdatalen)
						startpos += hdrdatalen
						self.datastate = 0
	...
	setupFeedbacks(system) {
		...
		feedbacks['selected_dest'] = {
			...
			callback: (feedback) => {
				console.log(
					self.label +
						'::feedback:selected_dest: selected_dest: ' +
						self.selected_dest +
						', option dest:' +
		...
		feedbacks['selected_source'] = {
			...
			callback: (feedback) => {
				console.log(
					self.label +
						'::feedback:selected_source: selected_source: ' +
	...
	padLeft(nr, n, str) {
		if (n < String(nr).length) {
			...
			return s
		} else return Array(n - String(nr).length + 1).join(str || '0') + nr
	}
